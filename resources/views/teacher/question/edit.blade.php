...
@section('content')
    <style>
        ...
        .ML__container {
            min-height: 80px;
            width: 100% !important;
            /* Hozirgi 40px o'rniga kattaroq qiymat bering */
            font-size: 1.2em;
            /* Hozirgi 1.1em o'rniga kattaroq shrift o'lchamini bering */
            /* Qolgan mavjud stil qoidalari joyida qolsin */
        }
    ...
    <div class="form-container">
        <h2>Savolni tahrirlash (Grafik va rasm bilan)</h2>
        ...
        <form action="{{ route('teacher.question.update', $question->id) }}" method="POST" id="question-form">
            ...
            <input type="hidden" name="status" value="{{ $question->status ?? '1' }}"> {{-- Statusni saqlash --}}
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="question_text" id="question_text_hidden_input"
                    value="{{ old('question_text', $question->name ?? '') }}">
                @error('question_text')
            ...
            <div class="form-group image-upload-section">
                ...
                <input type="hidden" name="question_image_base64" id="question_image_base64_input"
                    value="{{ old('question_image_base64', $question->question_image_base64 ?? '') }}">
                <div class="image-preview-container"
                    style="display: {{ old('question_image_base64', $question->question_image_base64 ?? '') ? 'block' : 'none' }};">
                    <img id="image_preview" src="{{ old('question_image_base64', $question->question_image_base64 ?? '') }}"
                        alt="Rasm preview" class="image-preview">
            ...
            <div class="options-section">
                <h3>Variantlar:</h3>
                <div id="options-list">
                    {{-- Mavjud variantlarni tiklash --}}
                    @php
                        $options = old('options', json_decode($question->options ?? '[]', true));
                        $correctOptionId = old('correct_option_id', $question->correct_option_id ?? '');
                    @endphp
                    ...
                    @if (!empty($options))
                        @foreach ($options as $index => $option)
                            <div class="option-item" data-option-id="{{ $index }}">
                                ...
                                <input type="hidden" name="options[{{ $index }}][text]"
                                    id="option-{{ $index }}" value="{{ $option['name'] ?? '' }}">
                        ...
                        @endforeach
                    @else
                        {{-- Dastlabki 2 ta variant (JavaScript tomonidan qo'shiladi agar variantlar bo'lmasa) --}}
                    @endif
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // `old('options')` yoki mavjud `$question->options` bo'lmaganda 0 va 1 indekslardan boshlash
            let optionCounter =
                {{ count(old('options', json_decode($question->options ?? '[]', true))) > 0 ? count(old('options', json_decode($question->options ?? '[]', true))) - 1 : -1 }};
            ...
            // Agar sahifa `old('graph_state')` yoki `$question->graph_state` bilan yuklangan bo'lsa, grafikni avtomatik ochish va yuklash
            ...
            $('#question-form').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // $('#question-form')[0].reset(); // Tahrirlashda reset qilish shart emas
